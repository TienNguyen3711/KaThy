...
(() => {
    ...
    let photoFiles     = [];
    let photoTextures  = [];
    let photoDelays    = [];   // ms after start for each photo
    let galleryDelay   = 999999;
    let photoPositions = [];   // THREE.Vector3[]
    let photoRevealed    = [];
    let photoEmergePos   = [];
    let photoRevealTimes = [];   // ms timestamp khi mỗi cloud photo được reveal
    ...
    const CFG = {
        ...
        innerRadius  : 27,
        rings: [
            { r:75, tube:3.5, count:550, tiltX: 7,  tiltZ:  0, tex:"cyan", hexColor:0x00CCFF, speed:0.0022 },
            { r:86, tube:2.5, count:480, tiltX:44,  tiltZ: 14, tex:"blue", hexColor:0x0088FF, speed:0.0048 },
            { r:66, tube:2.8, count:450, tiltX:-27, tiltZ: -9, tex:"ice",  hexColor:0xAADDFF, speed:0.0068 },
        ],
        galleryCount   : 650000,
        gallerySize    : 0.55,
        galleryZJitter : 1,
    ...
    // Grid 3-4-5 trực diện + cloud positions phía ngoài quả cầu
    function computePhotoPositions(n) {
        const layout   = [3, 4, 5];
        const xGap     = 22, yGap = 22, z = -40;
        const positions = [];
        let idx = 0;
        ...
        // Cloud positions: scattered around sphere exterior (radius 90–130)
    ...
    async function imageToPointTargets(url, count, cx, cy) {
        ...
        // Resize xuống max 1000px để tránh OOM với ảnh lớn (e.g. 6000x3375)
        const MAX_DIM = 1500;
        ...
        const SCREEN_W = SCREEN_H * (camera ? camera.aspect : 16 / 9);

        // Thu thập pixel opaque + tính bounding box + centroid trong 1 lần quét
        const pts = [];
        ...
        // Dùng centroid & bounding box thực tế → tránh lệch khi có vùng transparent lớn
        ...
        // Scale theo nội dung thực (bounding box), fill ~55% khung hình
        const adjScale = Math.min((SCREEN_H * 0.55) / ch, (SCREEN_W * 0.55) / cw);
    ...
    async function init3D() {
        ...
        photoScene = new THREE.Scene(); // ảnh render riêng, không bị particle đè
        ...
        // Tạo photo meshes dựa trên danh sách động
        ...
        // "For Ka Thy" text — vẽ khi animate (reveal trái→phải)
        ...
        msgMesh.position.set(0, -34, 0); msgMesh.visible = false;
        scene.add(msgMesh);

        // Subtitle
        await document.fonts.load('700 58px "Dancing Script"');
    ...
    async function buildGalleryTargets() {
        ...
        try {
            ...
            console.log(`[Gallery] Build OK — ${count} particles, image: ${heartFiles[0]}`);
        } catch (e) {
            console.error("[Gallery] Build FAILED:", e); galleryReady = false;
    ...
    function animate() {
        ...
        if (isGallery) {
            ...
            // Reveal chữ trái→phải như viết tay (1.6 giây)
        ...
        // Khi ảnh đầu tiên xuất hiện: camera phóng xuyên vào lõi cầu + xoay chậm lại
        ...
        // ── Photo timeline (grid photos only; cloud photos revealed in cloud phase) ──
        photoDelays.forEach((delay, i) => {
            if (delay === Infinity) return;
            if (elapsed >= delay && !photoRevealed[i]) {
                photoRevealed[i] = true;
                // Điểm xuất phát: bề mặt quả cầu
                const theta = Math.random() * Math.PI * 2;
                const phi   = Math.acos(2 * Math.random() - 1);
        ...
        if (finaleStarted) {
            ...
            const progress = Math.min(ft / 3500, 1.0); // 0→1 trong 3.5s

            // Ảnh hội tụ về tâm và biến mất
            ...
            // Camera lùi về vị trí xem gallery (reset cả x/y từ cloud orbit)
            ...
            // Quả cầu tăng tốc dần từ bình thường → nhanh dần rồi tan thành ảnh
            const accelTarget = 1.0 + Math.pow(progress, 1.5) * 17.0;
            rotMult = Math.min(rotMult + (accelTarget - rotMult) * 0.05, 18.0);

            // Sau 3.5s: sphere nổ → particles tạo thành Final.png
            if (progress >= 1.0 && !galleryTriggered) {
                ...
                if (groupGallery?.geometry) {
                    ...
                    // Particles xuất phát từ bề mặt sphere (hiệu ứng nổ)
                    for (let i = 0; i < CFG.galleryCount; i++) {
                        const phi   = Math.acos(2 * Math.random() - 1);
                        const theta = Math.random() * Math.PI * 2;
            ...
            // ── CLOUD PHASE: ký ức nổi trôi — photos float in 3D, camera orbits ──
            const ct = Date.now() - cloudStartTime;
            const CLOUD_DURATION = 28000; // 28 seconds of orbiting

            // Caption: fade in sau 3s, fade out 4s trước khi kết thúc
            ...
            const ORBIT_R = 125;
            const orbitAngle = ct * 0.001 * 0.055; // slow orbit

            // Camera orbits around origin, with gentle vertical bob
            ...
            // Stagger-reveal cloud photos — bắn ra từ bề mặt sphere như một hạt particle
            for (let i = MAX_PHOTOS; i < photoMeshes.length; i++) {
                const ci = i - MAX_PHOTOS;
                if (ct >= ci * 1400 && !photoRevealed[i]) {
                    photoRevealed[i]    = true;
                    photoRevealTimes[i] = Date.now();
                    // Emerge từ sphere surface CÙNG hướng với cloud target → luôn bắn ra ngoài
            ...
            // Float all revealed photos — cloud photos bắn ra nhanh rồi mới float nhẹ
            ...
            photoMeshes.forEach((mesh, i) => {
                ...
                // Age-based lerp: nhanh khi mới bắn, chậm khi đã ổn định
                let lerpF;
            ...
            // Bình thường: grid ảnh float tới vị trí lưới
            ...
            // Trigger cloud phase sau khi tất cả grid photos đã xuất hiện + 2s
            ...
            if (allGridOut && elapsed >= lastGridDelay + 2000 && !cloudStarted) {
                ...
                // Scatter grid photos to random cloud positions
        ...
        renderer.render(scene, camera);
        // Render ảnh trên cùng — tách hoàn toàn khỏi depth buffer của particles
    ...
    async function startSystem() {
        ...
        // Grid photos: staggered delays. Cloud photos: revealed by cloud phase.
        let t = 3000;
        ...
        galleryDelay = 999999; // gallery tắt — finale sẽ handle kết thúc
